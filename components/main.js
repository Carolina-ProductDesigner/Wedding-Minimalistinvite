...
/* ─────────────────────────────────────────
   SCROLL REVEAL (mobile historia)
───────────────────────────────────────── */
function initScrollReveal() {
  const hm = document.getElementById('historiaMobile');
  if (!hm) return;

  const items = hm.querySelectorAll('.sr');
  ...
  const observer = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        // once visible, stop observing (stays visible on scroll back up too)
        observer.unobserve(entry.target);
      }
    });
  }, {
    root: hm,          // scroll root is the .hm container
    threshold: 0.12,
