...
/* ═══════════════════════════════════════════════
   MOBILE SCROLL REVEAL — GSAP-powered
   Smooth entrance for all elements as they scroll into view.
   Uses IntersectionObserver to trigger GSAP tweens.
   ═══════════════════════════════════════════════ */
function initScrollRevealMobile() {
  ...
  // ── Historia: observe within section-0 ──
  const sec0 = document.getElementById('section-0');
  if (sec0) observeReveal(sec0, '.sr, .sr-img, .sr-line', useGsap);

  // ── La Boda: tag and observe within section-1 ──
  const sec1 = document.getElementById('section-1');
  if (sec1) {
    sec1.querySelectorAll('.boda__title, .boda__date, .boda__venue, .boda__address, .boda__dresscode-inline, .btn, .boda__divider, .boda__itinerario-title, .boda__timeline li, .boda__dresscode-title, .boda__dresscode-desc, .boda__photo-col').forEach((el, i) => {
      if (!el.classList.contains('sr')) {
        el.classList.add('sr');
        el.style.transitionDelay = `${Math.min(i * 0.05, 0.4)}s`;
      }
    });
    observeReveal(sec1, '.sr', useGsap);
  }

  // ── Other sections ──
  document.querySelectorAll('.section:not(.section--historia):not(.section--eventos)').forEach(sec => {
    sec.querySelectorAll('.hotel__card, .regalo__card, .otras-opciones, .contacto__block, .rsvp__form > *, .section__title, .section__subtitle').forEach((el, i) => {
      if (!el.classList.contains('sr')) {
        el.classList.add('sr');
        el.style.transitionDelay = `${Math.min(i * 0.06, 0.5)}s`;
      }
    });
    observeReveal(sec, '.sr', useGsap);
  });
}

function observeReveal(scrollRoot, selector, useGsap) {
  ...
  const obs = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      ...
      if (useGsap) {
        ...
        if (el.classList.contains('sr-img')) {
          // Image curtain wipe
          gsap.fromTo(el,
            { clipPath: 'inset(100% 0 0 0)', scale: 1.06 },
            { clipPath: 'inset(0% 0 0 0)', scale: 1, duration: 1.1, delay, ease: 'power3.out' }
          );
        } else if (el.classList.contains('sr-line')) {
          // Text line mask
          const inner = el.querySelector('.sr-line-inner');
          if (inner) gsap.fromTo(inner,
            { y: '105%' },
            { y: '0%', duration: 0.9, delay, ease: 'power3.out' }
          );
        } else {
          // Generic fade + rise
          gsap.fromTo(el,
  ...
  }, { root: scrollRoot, threshold: 0.06, rootMargin: '0px 0px -20px 0px' });
