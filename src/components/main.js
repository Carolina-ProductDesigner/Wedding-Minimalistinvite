...
/* ═══════════════════════════════════════════════
   DESKTOP: HOVER SHOOT + GENTLE FLOAT
   Each photo has its own shoot direction.
   On mouseenter → photo flies dramatically.
   On mouseleave → floats back smoothly.
   CSS keyframe handles idle float animation.
   ═══════════════════════════════════════════════ */
function initHoverFloat() {
  const section = document.getElementById('section-0');
  if (!section) return;

  // Unique shoot vector + magnitude for each figure
  const configs = {
    'hd__fig--main':    { x: -120, y: -80,  rot:  3 },
    'hd__fig--center':  { x:   60, y:  100, rot: -2 },
    'hd__fig--rtop':    { x:  100, y: -90,  rot:  2 },
    'hd__fig--rbottom': { x: -80,  y:  110, rot: -3 },
  };

  section.querySelectorAll('.hd__fig').forEach(fig => {
    // Find which config applies
    const key = Object.keys(configs).find(cls => fig.classList.contains(cls));
    const cfg = configs[key] || { x: 80, y: -60, rot: 2 };

    fig.addEventListener('mouseenter', () => {
      fig.classList.remove('returning');
      fig.classList.add('shooting');
      fig.style.transform = `translate(${cfg.x}px, ${cfg.y}px) rotate(${cfg.rot}deg) scale(1.04)`;
    });
    ...
    fig.addEventListener('mouseleave', () => {
      ...
      fig.style.transform = '';
      setTimeout(() => fig.classList.remove('returning'), 1200);
    });
  });
}
